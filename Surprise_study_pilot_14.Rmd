...
```{r echo=FALSE, warning=FALSE, message=FALSE}
df14_raw <- read.csv("/Users/marjan/Desktop/aim_lab_1/SUP_PRF_pilot_vid_big_PE_avatars/SUP_PRF_pilot_vid_big_PE_avatars_v9/SUP_PRF_pilot_vid_big_PE_avatars_v9_task_main.csv")
df14_spin <- read.csv("/Users/marjan/Desktop/aim_lab_1/SUP_PRF_pilot_vid_big_PE_avatars/SUP_PRF_pilot_vid_big_PE_avatars_v9/SUP_PRF_pilot_vid_big_PE_avatars_v9_mini_spin.csv")
df14_cesd <- read.csv("/Users/marjan/Desktop/aim_lab_1/SUP_PRF_pilot_vid_big_PE_avatars/SUP_PRF_pilot_vid_big_PE_avatars_v9/SUP_PRF_pilot_vid_big_PE_avatars_v9_ces_d.csv")
```
...
df14$m_hist <- ifelse(df14$Spreadsheet..Histogram == "h1.png", 20,
                      ifelse(df14$Spreadsheet..Histogram == "h2.png", 212,
...
# 
# It seems the relationship between prediction and histogram is also weaker than before. 

# corr_test <- final_df14 %>%
#   group_by(Random_ID) %>%
#   summarise(correlation = cor(Response_m_hist, Response_pred, use = "complete.obs")) %>%
#   mutate(label = paste("R = ", round(correlation, digits = 2)))
# 
# # calculate the average correlation across all individuals and print it
# average_correlation <- mean(corr_test$correlation, na.rm = TRUE)
# message_to_print_H <- paste("average correlation between histogram mean and prediction:", average_correlation)
# 
# print(message_to_print_H)
# 
# ggplot(final_df14, aes(x = Response_pred, y = Response_m_hist, color = Social_Anxiety, group= Social_Anxiety)) +
#   geom_point(aes(color=Social_Anxiety, alpha = 0.3)) +
#   geom_smooth(method = "lm", color="blue", aes(group=Random_ID)) + 
#   # labs(color="Legend") +
#   theme_minimal() +
...
#   ylab("Histogram mean") +
#     facet_wrap(~ Random_ID, scales = "free") +
#     geom_text(data=corr_test, aes(x=Inf, y=Inf, label=label, group=Random_ID), hjust=1.1, vjust=1.1, size=2.7, fontface = "bold", inherit.aes=FALSE) +
#     theme(strip.text = element_text(size = 6),
#         axis.text.x = element_text(size = 5.5),  
#         axis.text.y = element_text(size = 5.5),  
#         strip.text.x = element_text(face = "bold"),  
...
avatar_feedback <- filter(avatar_feedback, Screen.ID == "tuhtwy" & Response.Type == "response")
