...
model {
 intercept  ~ normal(0, 1);
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      Response_Ax[i, t] ~ normal(w0[i] + w1[i] * rpe_sum + w2[i] * Response_fdbk[i, t], sig[i]);
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      log_lik[i] += normal_lpdf(Response_Ax[i, t] | w0[i] + w1[i] * rpe_sum + w2[i] * Response_fdbk[i, t], sig[i]);
      y_pred[i, t] = normal_rng(w0[i] + w1[i] * rpe_sum + w2[i] * Response_fdbk[i, t], sig[i]);
