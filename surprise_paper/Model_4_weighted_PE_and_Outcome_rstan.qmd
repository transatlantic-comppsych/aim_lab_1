...
## Weighted cumulative PE + outcome (not cumulative nor weighted)

In this model, anxiety/mood on trial $t$ is calculated by adding the intercept $\beta_0$ (the value of anxiety/mood when outcome and weighted sum of PEs on that trial are set to 0), outcome, and weighted sum of PEs on trial $t$.

The explicit expectation is the one reported by the participants.

$$
A_t = \beta_0 + \beta_1 \cdot O_t+ \beta_2 \cdot \sum_{j=1}^{t} \gamma^{t - j} \cdot PE_j 
$$

$$
M_t = \beta_0 + \beta_1 \cdot O_t+ \beta_2 \cdot \sum_{j=1}^{t} \gamma^{t - j} \cdot PE_j 
$$ $$
PE_t = O_t - Ee_t 
$$
...
weighted_PE_outcome_anx_fit <- stan(model_code = weighted_PE_outcome_anx, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

# Print results
print(weighted_PE_outcome_anx_fit)

# Save the model output as an .rds file
#saveRDS(weighted_PE_outcome_anx_fit, file = "weighted_PE_outcome_anx_fit.rds")
...
# Plot results
traceplot(weighted_PE_outcome_anx_fit)

# Extract posterior samples
posterior_samples <- rstan::extract(weighted_PE_outcome_anx_fit)
...
log_lik_vector <- rstan::extract(weighted_PE_outcome_anx_fit, pars = "log_lik")$log_lik  # Dimensions: [S, N] (S = posterior samples)
...
data <- data_merged
#data[, c("Response_fdbk", "Response_SubjPE", "Response_H")] <- scale(data[, c("Response_fdbk", "Response_SubjPE", "Response_H")])
...
model {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      Response_H[i, t] ~ normal(w0[i] + w1[i] * rpe_sum + w2[i] * Response_fdbk[i, t], sig[i]);
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      log_lik[i] += normal_lpdf(Response_H[i, t] | w0[i] + w1[i] * rpe_sum + w2[i] * Response_fdbk[i, t], sig[i]);
      y_pred[i, t] = normal_rng(w0[i] + w1[i] * rpe_sum + w2[i] * Response_fdbk[i, t], sig[i]);
...
# Compile and Fit the Model
weighted_PE_outcome_mood_fit <- stan(model_code = weighted_PE_outcome_mood, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

options(max.print = 9999)  # Set a higher limit

# Print results
print(weighted_PE_outcome_mood_fit)

# Save the model output as an .rds file
#saveRDS(weighted_PE_outcome_mood_fit, file = "weighted_PE_outcome_mood_fit.rds")
...
# Plot results
rstan::traceplot(weighted_PE_outcome_mood_fit)

# Extract posterior samples
posterior_samples <- rstan::extract(weighted_PE_outcome_mood_fit)
...
# Extract log-likelihood per participant (N-length vector)
log_lik_vector <- rstan::extract(weighted_PE_outcome_mood_fit, pars = "log_lik")$log_lik  # Dimensions: [S, N] (S = posterior samples)
...
# Stan Model
weighted_PE_outcome_mood_standardized <-  "
data {
  int<lower=1> N; // Number of Participants
  int<lower=1> T; // Trials per subject
  int<lower=1, upper=T> Tsubj[N]; 
...
generated quantities {
  ...
  real sd_Response_SubjPE;
  real sd_Response_fdbk;
  real sd_Response_H;
  ...
  // Compute standard deviations of predictors and response
  sd_Response_SubjPE = sd(to_vector(Response_SubjPE));
  sd_Response_fdbk = sd(to_vector(Response_fdbk));
  sd_Response_H = sd(to_vector(Response_H));
  
  // Compute standardized coefficients
  w1_std = mu_w1 * (sd_Response_SubjPE / sd_Response_H);
  w2_std = mu_w2 * (sd_Response_fdbk / sd_Response_H);
...
# Compile and Fit the Model
weighted_PE_outcome_mood_standardized_fit <- stan(model_code = weighted_PE_outcome_mood_standardized, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

options(max.print = 9999)  # Set a higher limit

# Print results
print(weighted_PE_outcome_mood_standardized_fit)
```

```{r}

# Plot results
rstan::traceplot(weighted_PE_outcome_mood_standardized_fit)

# Extract posterior samples
posterior_samples <- rstan::extract(weighted_PE_outcome_mood_standardized_fit)
...
# Extract log-likelihood per participant (N-length vector)
log_lik_vector <- rstan::extract(weighted_PE_outcome_mood_standardized_fit, pars = "log_lik")$log_lik  # Dimensions: [S, N] (S = posterior samples)
