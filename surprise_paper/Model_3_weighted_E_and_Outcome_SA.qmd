---
title: "Model 3 (with SA grouping covariate)"
format: html
editor: visual
---
...
Social_Anxiety_vector <- tapply(data$mini_SPIN_total, data$Random_ID, function(x) unique(x)[1])
...
stan_data <- list(
  ...
  Social_Anxiety = as.vector(Social_Anxiety_vector)  # vector of length N
)


# Stan Model
weighted_exp_outcome_anx_SA <- "
data {
...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    w2[i]  = w2_mu + beta_w2 * Social_Anxiety[i] + sigma[2] * w2_pr[i];

    gam[i] = Phi_approx(gam_mu + beta_gam * Social_Anxiety[i] + sigma[3] * gam_pr[i]);
    sig[i] = exp(sig_mu + beta_sig * Social_Anxiety[i] + sigma[4] * sig_pr[i]);
  }
}
model {
  intercept  ~ normal(50, 10);
  w1_mu      ~ normal(0, 1.0);
  w2_mu      ~ normal(0, 1.0);
  gam_mu     ~ normal(0, 1.0);
  sig_mu     ~ normal(0, 1.0);

  // Priors for regression weights
  beta_w0  ~ normal(0, 1.0);
  beta_w1  ~ normal(0, 1.0);
  beta_w2  ~ normal(0, 1.0);
  beta_gam ~ normal(0, 1.0);
  beta_sig ~ normal(0, 1.0);

  sigma_w0 ~ normal(0, 10);
  sigma    ~ normal(0, 0.2);
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    log_lik[i] = 0;
    for (t in 1:Tsubj[i]) {
      real mu = w0[i] + w1[i] * ev_sum + w2[i] * Response_fdbk[i, t];
...
weighted_exp_outcome_anx_SA_fit <- stan(model_code = weighted_exp_outcome_anx_SA, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

options(max.print = 99999)
# Print results
print(weighted_exp_outcome_anx_SA_fit)

# Save the model output as an .rds file
saveRDS(weighted_exp_outcome_anx_SA_fit, file = "weighted_exp_outcome_anx_SA_fit.rds")
...
# Plot results
traceplot(weighted_exp_outcome_anx_SA_fit)

# Extract posterior samples
posterior_samples <- rstan::extract(weighted_exp_outcome_anx_SA_fit)
...
log_lik_vector <- rstan::extract(weighted_exp_outcome_anx_SA_fit, pars = "log_lik")$log_lik  # Dimensions: [S, N] (S = posterior samples)
...
posterior <- rstan::extract(weighted_exp_outcome_anx_SA_fit)
...
# Same for w2
w2_low  <- rowMeans(matrix(posterior$w2_mu, nrow = n_samples, ncol = length(low_SA_vals)) +
                    posterior$beta_w2 %o% low_SA_vals)

w2_high <- rowMeans(matrix(posterior$w2_mu, nrow = n_samples, ncol = length(high_SA_vals)) +
                    posterior$beta_w2 %o% high_SA_vals)
...
SA_scores <- data %>%
  group_by(Random_ID) %>%
  summarise(SA = first(mini_SPIN_total)) %>%
  arrange(Random_ID)
...
corr_results <- rbind(
  ...
  get_corr_stats(param_df$SA, param_df$w2),
...
```{r}
 #Compile and Fit the Model
weighted_exp_outcome_mood_SA_fit <- stan(model_code = weighted_exp_outcome_mood_SA, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

options(max.print = 99999)
# Print results
print(weighted_exp_outcome_mood_SA_fit)

# Save the model output as an .rds file
saveRDS(weighted_exp_outcome_mood_SA_fit, file = "weighted_exp_outcome_mood_SA_fit.rds")
...
# Plot results
traceplot(weighted_exp_outcome_mood_SA_fit)

# Extract posterior samples
posterior_samples <- rstan::extract(weighted_exp_outcome_mood_SA_fit)
...
# Extract log-likelihood per participant (N-length vector)
log_lik_vector <- rstan::extract(weighted_exp_outcome_mood_SA_fit, pars = "log_lik")$log_lik  # Dimensions: [S, N] (S = posterior samples)
...
posterior <- rstan::extract(weighted_exp_outcome_mood_SA_fit)
...
w0_high <- rowMeans(matrix(posterior$intercept, nrow = n_samples, ncol = length(high_SA_vals)) + posterior$beta_w0 %o% high_SA_vals)
...
data$mini_SPIN_total <- scale(data$mini_SPIN_total)
...
Model_3_anx <- lmerTest::lmer(
  Response_Ax ~  weighted_cumulative_exp_exp*mini_SPIN_total  +  Response_fdbk*mini_SPIN_total+ (1 + weighted_cumulative_exp_exp + Response_fdbk  | Random_ID),
  data = data)

Model_3_mood <- lmerTest::lmer(
  Response_H ~ weighted_cumulative_exp_exp*mini_SPIN_total  +  Response_fdbk*mini_SPIN_total+ (1 + weighted_cumulative_exp_exp + Response_fdbk  | Random_ID),
  data = data)
...
# Merge with mini_SPIN_total (as before)
spin_scores <- data[!duplicated(data$Random_ID), c("Random_ID", "mini_SPIN_total")]
merged_df <- merge(rand_eff, spin_scores, by = "Random_ID")
...
merged_df$SPIN_group <- ifelse(merged_df$mini_SPIN_total >= 6, "Above cutoff", "Below cutoff")
merged_df$SPIN_group <- factor(merged_df$SPIN_group, levels = c("Below cutoff", "Above cutoff"))

ggplot(merged_df, aes(x = mini_SPIN_total, y = eff_slope_wce)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "Participant-Specific Slopes for weighted_cumulative_exp_exp",
       x = "mini_SPIN_total", y = "Effective Slope")

ggplot(merged_df, aes(x = mini_SPIN_total, y = eff_slope_feedback)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "Participant-Specific Slopes for Response_fdbk",
       x = "mini_SPIN_total", y = "Effective Slope")
cor.test(merged_df$eff_slope_feedback, merged_df$mini_SPIN_total)

ggplot(merged_df, aes(x = mini_SPIN_total, y = eff_slope_feedback, color = SPIN_group)) +
  geom_point(size = 3, alpha = 0.8) +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  scale_color_manual(values = c("Below cutoff" = "steelblue", "Above cutoff" = "firebrick")) +
  labs(title = "Effective Slope of Response_fdbk vs mini_SPIN_total",
       x = "mini_SPIN_total",
       y = "Effective Slope",
       color = "SPIN Group") +
  theme_minimal()
...
$$
Anxiety_{ti} = A_{0i} + \beta_{Ei} \cdot \sum_{j=1}^{t} \gamma^{t - j} \cdot E_{ji} + \beta_{Oi} \cdot O_{ti} + \beta_{SA} \cdot SA_i + \beta_{E \times SA} \cdot SA_i \cdot \left( \sum_{j=1}^{t} \gamma^{t - j} \cdot E_{ji} \right) + \beta_{O \times SA} \cdot SA_i \cdot O_{ti}
$$
...
weighted_exp_outcome_anx_SA_main_effect <- "
data {
  int<lower=1> N; // Number of Participants
  int<lower=1> T; // Trials per subject
  int<lower=1, upper=T> Tsubj[N]; 
...
model {
  ...
  beta_SA  ~ normal(0, 0.5);  // Now scaled, so smaller prior makes sense
  ...
  for (i in 1:N) {
    real ev_sum = 0;
    for (t in 1:Tsubj[i]) {
      Response_Ax[i, t] ~ normal(w0[i] + w1[i] * ev_sum + w2[i] * Response_fdbk[i, t] + beta_SA * Social_Anxiety[i], sig[i]);
      ev_sum += Response_pred[i, t];
      ev_sum *= gam[i];
    }
  }
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    log_lik[i] = 0;
    for (t in 1:Tsubj[i]) {
      real mu = w0[i] + w1[i] * ev_sum + w2[i] * Response_fdbk[i, t] + beta_SA * Social_Anxiety[i];
      log_lik[i] += normal_lpdf(Response_Ax[i, t] | mu, sig[i]);
      y_pred[i, t] = normal_rng(mu, sig[i]);
...
# Compile and Fit the Model
weighted_exp_outcome_anx_SA_main_effect_fit <- stan(model_code = weighted_exp_outcome_anx_SA_main_effect, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

options(max.print = 99999)
# Print results
print(weighted_exp_outcome_anx_SA_main_effect_fit)

# Save the model output as an .rds file
saveRDS(weighted_exp_outcome_anx_SA_main_effect_fit, file = "weighted_exp_outcome_anx_SA_main_effect_fit.rds")
...
# Plot results
traceplot(weighted_exp_outcome_anx_SA_main_effect_fit)

# Extract posterior samples
posterior_samples <- rstan::extract(weighted_exp_outcome_anx_SA_main_effect_fit)
...
# Extract log-likelihood per participant (N-length vector)
log_lik_vector <- rstan::extract(weighted_exp_outcome_anx_SA_main_effect_fit, pars = "log_lik")$log_lik  # Dimensions: [S, N] (S = posterior samples)
...
posterior <- rstan::extract(weighted_exp_outcome_anx_SA_main_effect_fit)
...
$$
Mood_{ti} = M_{0i} + \beta_{Ei} \cdot \sum_{j=1}^{t} \gamma^{t - j} \cdot E_{ji} + \beta_{Oi} \cdot O_{ti} + \beta_{SA} \cdot SA_i + \beta_{E \times SA} \cdot SA_i \cdot \left( \sum_{j=1}^{t} \gamma^{t - j} \cdot E_{ji} \right) + \beta_{O \times SA} \cdot SA_i \cdot O_{ti}
$$
...
weighted_exp_outcome_mood_SA_main_effect <- "
data {
  int<lower=1> N; // Number of Participants
  int<lower=1> T; // Trials per subject
  int<lower=1, upper=T> Tsubj[N]; 
...
model {
  ...
  for (i in 1:N) {
    real ev_sum = 0;
    for (t in 1:Tsubj[i]) {
      Response_H[i, t] ~ normal(w0[i] + w1[i] * ev_sum + w2[i] * Response_fdbk[i, t] + beta_SA * Social_Anxiety[i], sig[i]);
      ev_sum += Response_pred[i, t];
      ev_sum *= gam[i];
    }
  }
...
```{r}
# Compile and Fit the Model
weighted_exp_outcome_mood_SA_main_effect_fit <- stan(model_code = weighted_exp_outcome_mood_SA_main_effect, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

options(max.print = 99999)
# Print results
print(weighted_exp_outcome_mood_SA_main_effect_fit)

# Save the model output as an .rds file
saveRDS(weighted_exp_outcome_mood_SA_main_effect_fit, file = "weighted_exp_outcome_mood_SA_main_effect_fit.rds")
...
# Plot results
traceplot(weighted_exp_outcome_mood_SA_main_effect_fit)

# Extract posterior samples
posterior_samples <- rstan::extract(weighted_exp_outcome_mood_SA_main_effect_fit)
...
# Extract log-likelihood per participant (N-length vector)
log_lik_vector <- rstan::extract(weighted_exp_outcome_mood_SA_main_effect_fit, pars = "log_lik")$log_lik  # Dimensions: [S, N] (S = posterior samples)
...
posterior <- rstan::extract(weighted_exp_outcome_mood_SA_main_effect_fit)
