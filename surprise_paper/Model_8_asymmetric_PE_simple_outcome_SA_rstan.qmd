...
data$mini_SPIN_total <- scale(data$mini_SPIN_total)[, 1]
...
# Stan Model
asymmPE_simple_Outcome_anx_SA <- "data {
  int<lower=1> N;
  int<lower=1> T;
...
asymmPE_simple_Outcome_anx_SA_fit <- stan(model_code = asymmPE_simple_Outcome_anx_SA, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

options(max.print = 9999)  

# Print results
print(asymmPE_simple_Outcome_anx_SA_fit)

saveRDS(asymmPE_simple_Outcome_anx_SA_fit, "asymmPE_simple_Outcome_anx_SA_fit.rds")
```

```{r}

# Plot results
traceplot(asymmPE_simple_Outcome_anx_SA_fit)

# Extract posterior samples
posterior_samples <- rstan::extract(asymmPE_simple_Outcome_anx_SA_fit)
...
log_lik_vector <- rstan::extract(asymmPE_simple_Outcome_anx_SA_fit, pars = "log_lik")$log_lik  # Dimensions: [S, N] (S = posterior samples)
...
posterior <- rstan::extract(asymmPE_simple_Outcome_anx_SA_fit)
...
# Stan Model
asymmPE_simple_Outcome_mood_SA <- "data {
  int<lower=1> N;
  int<lower=1> T;
  int<lower=1, upper=T> Tsubj[N]; 
  array[N, T] real Response_fdbk;
...
# Compile and Fit the Model
asymmPE_simple_Outcome_mood_SA_fit <- stan(model_code = asymmPE_simple_Outcome_mood_SA, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

options(max.print = 9999)  # Set a higher limit

# Print results
print(asymmPE_simple_Outcome_mood_SA_fit)

saveRDS(asymmPE_simple_Outcome_mood_SA_fit, "asymmPE_simple_Outcome_mood_SA_fit.rds")
```

```{r}
# Plot results
traceplot(asymmPE_simple_Outcome_mood_SA_fit)

# Extract posterior samples
posterior_samples <- rstan::extract(asymmPE_simple_Outcome_mood_SA_fit)
...
# Extract log-likelihood per participant (N-length vector)
log_lik_vector <- rstan::extract(asymmPE_simple_Outcome_mood_SA_fit, pars = "log_lik")$log_lik  # Dimensions: [S, N] (S = posterior samples)
...
```{r}
posterior <- rstan::extract(asymmPE_simple_Outcome_mood_SA_fit)
...
```{r}
asymmPE_simple_Outcome_mood_SA_fit<- readRDS("asymmPE_simple_Outcome_mood_SA_fit.rds")
library(ggplot2)

plot_gamma_SA <- function(df, color = "#FF7F00", filename = "plot_gamma_SA.png") {
  # Compute Pearson correlation
  test <- cor.test(df$SA, df$gamma, method = "pearson")
  r_value <- round(test$estimate, 3)
  p_value <- round(test$p.value, 2)

  # Create plot
  p <- ggplot(df, aes(x = SA, y = gamma)) +
    geom_point(color = color, alpha = 0.4, size = 3) +         # more transparent points
...
# Example usage
plot_gamma <- plot_gamma_SA(param_df)
