---
title: "Recency model - model 14"
format: html
editor: visual
---

## Recency Model (Weighted sum of expectations + weighted sum of PE)
...
$$
A_t = \beta_0 +\beta_1\cdot W_{1t} + \beta_2\cdot W_{2t} \
$$

$$
M_t = \beta_0 +\beta_1\cdot W_{1t} + \beta_2\cdot W_{2t} \
$$

$$
W_{1t} = \sum_{j=1}^{t} \gamma^{t - j} \cdot PE(j)
$$

$$
W_{2t} = \sum_{j=1}^{t} \gamma^{t - j} \cdot E(j)
$$
...
recency_anx_fit <- stan(model_code = recency_anx, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

# Print results
print(recency_anx_fit)

# Save the model output as an .rds file
saveRDS(recency_anx_fit, file = "recency_anx_fit_test.rds")
```

```{r}

# Plot results
traceplot(recency_anx_fit)

# Extract posterior samples
posterior_samples <- rstan::extract(recency_anx_fit)
...
log_lik_vector <- rstan::extract(recency_anx_fit, pars = "log_lik")$log_lik  # Dimensions: [S, N] (S = posterior samples)
...
# Extract log-likelihood per participant (already done)
log_lik_vector <- rstan::extract(recency_anx_fit, pars = "log_lik")$log_lik  

# Compute the number of data points (N) and parameters (K)
N <- nrow(log_lik_vector)  # Number of participants (or observations)
K <- length(rstan::extract(recency_anx_fit)$beta)  # Number of model parameters (you might need to adjust this line based on your model)

# Calculate log-likelihood for each sample
log_lik_total <- apply(log_lik_vector, 2, sum)  # Sum of log-likelihood for each participant

# Compute BIC for each sample
bic <- -2 * log_lik_total + log(N) * K

# Compute the mean BIC across posterior samples (you can use the mean if you want a single BIC value)
mean_bic <- mean(bic)

# Print the BIC result
print(paste("Mean BIC:", mean_bic))
...
# Compile and Fit the Model
recency_mood_fit <- stan(model_code = recency_mood, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

options(max.print = 9999)  # Set a higher limit

# Print results
print(recency_mood_fit)

# Save the model output as an .rds file
saveRDS(recency_mood_fit, file = "recency_mood_fit.rds")
```

```{r}

# Plot results
rstan::traceplot(recency_mood_fit)

# Extract posterior samples
posterior_samples <- rstan::extract(recency_mood_fit)
...
# Extract log-likelihood per participant (N-length vector)
log_lik_vector <- rstan::extract(recency_mood_fit, pars = "log_lik")$log_lik  # Dimensions: [S, N] (S = posterior samples)
...
ggplot(plot_data, aes(x = Trial)) +
  ...
  # Extract log-likelihood per participant (already done)
log_lik_vector <- rstan::extract(recency_mood_fit, pars = "log_lik")$log_lik  

# Compute the number of data points (N) and parameters (K)
N <- nrow(log_lik_vector)  # Number of participants (or observations)
K <- length(rstan::extract(recency_mood_fit)$beta)  # Number of model parameters (you might need to adjust this line based on your model)
