...
pearson_r_ci <- function(x, y, conf.level = 0.95) {
  n <- length(x)
  r <- cor(x, y)
  ...
  ci_lower <- tanh(ci_lower_z)
  ci_upper <- tanh(ci_upper_z)
  return(c(r = r, lower = ci_lower, upper = ci_upper))
}

coverage_summary <- map_df(results, function(res) {
  df <- res$df
  r_ci <- pearson_r_ci(df$true, df$mean)
  tibble(
    covered        = mean(df$covered, na.rm = TRUE),
    corr           = r_ci["r"],
    corr_lower     = r_ci["lower"],
    corr_upper     = r_ci["upper"]
  )
}, .id = "param")
...
```{r DEP distribution}
#| label: tbl-DEP-distribution
#| tbl-cap: "Distribution of depression scores by participant group. Depression symptoms were assessed using the CES-D for all groups except school participants aged 14–18, who were assessed using the RCADS. The CES-D scores range from 0 to 60, while RCADS scores range from 0 to 30. Depression categories were determined using standard cutoffs: scores ≥16 on the CES-D and gender- and age-specific thresholds on the RCADS."
