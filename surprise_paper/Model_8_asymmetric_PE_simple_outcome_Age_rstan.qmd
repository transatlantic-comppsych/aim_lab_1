...
# Load Data
data <- read.csv("/Users/elenabagdades/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Desktop/GitHub/aim_lab_1/Surprise_task_data_analysis/merged_data_w_demographics.csv")
...
saveRDS(asymmPE_simple_Outcome_anx_Age_fit, "asymmPE_simple_Outcome_anx_Age_fit.rds")
```
...
asymmPE_simple_Outcome_anx_Age_fit <- readRDS("/Users/elenabagdades/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Desktop/R code/Computational modelling/Full_scale_data_comp_modelling_results/asymmPE_simple_Outcome_anx_Age_fit.rds")
Age_vals <- stan_data$Age  # Vector of length N
posterior <- rstan::extract(asymmPE_simple_Outcome_anx_Age_fit)
...
Age_scores <- data %>%
  ...
  summarise(Age = first(Age), 
    dataset = first(Dataset)  # Add dataset here
  ) %>%
  arrange(Random_ID)
...
param_df <- data.frame(
  ...
  Age = Age_scores$Age,
  dataset = Age_scores$dataset,  # Add this line
  w0 = w0_mean,
  w1 = w1_mean,
  w_pos = w_pos_mean,
  w_neg = w_neg_mean,
...
plot_correlation <- function(df, param) {
  ggplot(df, aes_string(x = "Age", y = param, color = "dataset")) +
    geom_point(alpha = 0.7, size = 3) +
    geom_smooth(method = "lm", se = TRUE, color = "black", linetype = "dashed") +
    labs(
      x = "Age",
      y = paste0("Mean Posterior of ", param),
      title = paste("Age vs", param)
    ) +
...
corr_results <- rbind(
  get_corr_stats(param_df$Age, param_df$w0),
  get_corr_stats(param_df$Age, param_df$w1),
  get_corr_stats(param_df$Age, param_df$w_pos),
    get_corr_stats(param_df$Age, param_df$w_neg),
  get_corr_stats(param_df$Age, param_df$gamma),
  get_corr_stats(param_df$Age, param_df$sigma)
)
...
# Plot results
traceplot(asymmPE_simple_Outcome_mood_Age_fit)
...
Age_vals <- stan_data$Age  # Vector of length N
posterior <- rstan::extract(asymmPE_simple_Outcome_mood_Age_fit)
