...
if ("Random_ID_new" %in% names(data)) {
  names(data)[names(data) == "Random_ID_new"] <- "Random_ID"
}
...
# Recompile the model (you'll need to keep the model code)
#recompiled_model <- stan_model(model_code = asymmPE_simple_Outcome_anx)

#asymmPE_simple_Outcome_anx_fit@stanmodel <- recompiled_model

#asymmPE_Outcome_mood_fit_info <- list(
 # stanfit = asymmPE_simple_Outcome_anx_fit,
 # stanmodel = asymmPE_simple_Outcome_anx_fit@stanmodel,
#  data = stan_data
#)
...
# Recompile the model (you'll need to keep the model code)
#recompiled_model <- stan_model(model_code = asymmPE_simple_Outcome_mood)

#asymmPE_simple_Outcome_mood_fit@stanmodel <- recompiled_model

#asymmPE_Outcome_mood_fit_info <- list(
 # stanfit = asymmPE_simple_Outcome_mood_fit,
 # stanmodel = asymmPE_simple_Outcome_mood_fit@stanmodel,
#  data = stan_data
#)
...
model <- lmerTest::lmer(Response_H ~ Response_PosPE+Response_NegPE+Response_fdbk + (1+Response_PosPE+Response_NegPE+Response_fdbk |Random_ID), data = data)
summary(model)
```
...
library(ggplot2)
library(Metrics)
#This combination of gamma values were the best fitted
gamma <- 0.94


data$weighted_cumulative_PE_pos <- numeric(nrow(data))
data$weighted_cumulative_PE_neg <- numeric(nrow(data))
data$weighted_cumulative_fdbk <- numeric(nrow(data))


for (id in unique(data$Random_ID)) {
  # Extract the subset of data for the current participant
  subset_data <- data[data$Random_ID == id, ]
  ...
  for (t in 1:nrow(subset_data)) {
    ...
    W3 <- 0
    for (j in 1:t) {
      W1 <- W1 + subset_data$Response_PosPE[j] * gamma ^(t - j)
          W2 <- W2 + subset_data$Response_NegPE[j] * gamma ^(t - j)
                    W3 <- W3 + subset_data$Response_fdbk[j] * gamma ^(t - j)

    }
# Ensure the indices match
    trial_number <- subset_data$Trial.Number[t]
    row_index <- which(data$Random_ID == id & data$Trial.Number == trial_number)
    
    if (length(row_index) == 1) {
      data$weighted_cumulative_PE_pos[row_index] <- W1
      data$weighted_cumulative_PE_neg[row_index] <- W2
      data$weighted_cumulative_fdbk[row_index] <- W3
...
model <- lmerTest::lmer(Response_H ~ weighted_cumulative_PE_pos+weighted_cumulative_PE_neg+Response_fdbk + (1+weighted_cumulative_PE_pos+weighted_cumulative_PE_neg+Response_fdbk |Random_ID), data = data)
summary(model)
```
