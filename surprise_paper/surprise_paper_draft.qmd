...
data_summary <- data %>%
  group_by(SA_status_LSAS) %>%
  summarise(mean_Response_pred = mean(Response_pred, na.rm = TRUE))

library(dplyr)


per_person_means <- data %>%
  group_by(Random_ID, Social_Anxiety) %>%
  summarise(mean_Response_pred = mean(Response_pred, na.rm = TRUE), .groups = "drop")

library(ggplot2)

ggplot(per_person_means, aes(x = Social_Anxiety, y = mean_Response_pred)) +
  geom_violin(fill = "lightblue", alpha = 0.6) +
  geom_boxplot(width = 0.1, outlier.shape = NA, color = "darkblue") +
  labs(title = "Per-Person Mean Response_pred by SA_status_LSAS",
       x = "SA_status_LSAS Group",
       y = "Mean Response_pred") +
  theme_minimal()
