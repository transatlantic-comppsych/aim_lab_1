...
## Weighted cumulative Ee + PE (not cumulative nor weighted)
...
$$
A_t = \beta_0 + \beta_1 \cdot \sum_{j=1}^{t} \gamma^{t - j} \cdot Ee_j + \beta_2 \cdot PE_t 
$$

$$
M_t = \beta_0 + \beta_1 \cdot \sum_{j=1}^{t} \gamma^{t - j} \cdot Ee_j + \beta_2 \cdot PE_t 
$$
...
data <- data_merged
#data[, c("Response_pred", "Response_SubjPE", "Response_Ax")] <- scale(data[, c("Response_pred", "Response_SubjPE", "Response_Ax")])
...
weighted_exp_PE_anx_fit <- stan(model_code = weighted_exp_PE_anx, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

# Print results
print(weighted_exp_PE_anx_fit)


# Save the model output as an .rds file
#saveRDS(weighted_exp_PE_anx_fit, file = "weighted_exp_PE_anx_fit.rds")
```

```{r}

# Plot results
traceplot(weighted_exp_PE_anx_fit)

# Extract posterior samples
posterior_samples <- rstan::extract(weighted_exp_PE_anx_fit)
...
log_lik_vector <- rstan::extract(weighted_exp_PE_anx_fit, pars = "log_lik")$log_lik  # Dimensions: [S, N] (S = posterior samples)
...
# Load Data#data <- read.csv("/Users/elenabagdades/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Desktop/GitHub/aim_lab_1/Extracted_data/pilot_21_variables.csv")
...
data <- data_merged
#data[, c("Response_pred", "Response_SubjPE", "Response_H")] <- scale(data[, c("Response_pred", "Response_SubjPE", "Response_H")])
...
# Compile and Fit the Model
weighted_exp_PE_mood_fit <- stan(model_code = weighted_exp_PE_mood, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

options(max.print = 9999)  # Set a higher limit

# Print results
print(weighted_exp_PE_mood_fit)


# Save the model output as an .rds file
#saveRDS(weighted_exp_PE_mood_fit, file = "weighted_exp_PE_mood_fit.rds")
```

```{r}

# Plot results
rstan::traceplot(weighted_exp_PE_mood_fit)

# Extract posterior samples
posterior_samples <- rstan::extract(weighted_exp_PE_mood_fit)
...
# Extract log-likelihood per participant (N-length vector)
log_lik_vector <- rstan::extract(weighted_exp_PE_mood_fit, pars = "log_lik")$log_lik  # Dimensions: [S, N] (S = posterior samples)
