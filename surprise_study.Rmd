...
final_df3 <- inner_join(df_Anxious, df_Happy, by = c("Random_ID", "Trial.Number"))


# Calculate correlation by Random_ID for anxiety
correlations_Ax_H <- final_df3 %>%
  group_by(Random_ID) %>%
...
<br> <br> 
```{r echo=FALSE, warning=FALSE, message=FALSE}
df <- read.csv("/Users/marjan/Desktop/pilot_surprise_SeptOCT2023/Pilot_surprise/SUP_PRF_pilot_fdbk_bignrnd_typ_v3_task_main.csv")
...
# Determine the range for the data if we want all plots to have the same limits, to know what limits to choose
x_range <- range(df_pred$Response, na.rm = TRUE)
y_range <- range(df_pred$m_hist, na.rm = TRUE)
...
df_raw <- read.csv("/Users/marjan/Desktop/pilot_surprise_SeptOCT2023/Pilot_surprise/SUP_PRF_pilot_fdbk_bignrnd_typ_v3_task_main.csv")   # Prolific,non-random feedback, text only (no video/audio) with bigger PE, once per judge, 37 (40 for now) subjects
...
ggplot(subset_df, aes(x=Response_Ax, y=Response_H)) +
  geom_point(aes(color="Data Points", alpha = 0.5)) +
  geom_smooth(method = "lm", color="blue", aes(group=Random_ID)) +     
  labs(color="Legend") +
  facet_wrap(~ Random_ID, scales = "free") +
...
df_fdbk <- subset(df, Screen == "fdbk")
names(df_fdbk)[names(df_PE) == "Screen"] <- "Screen_fdbk"
names(df_fdbk)[names(df_PE) == "Response"] <- "Response_fdbk"
...
ggplot(df_fdbk_Ax_H, aes(x=Response, y=Response_Ax)) +
  geom_point(aes(color="Data Points", alpha = 0.5)) +
  geom_smooth(method = "lm", color="blue", aes(group=Random_ID)) +     
  labs(color="Legend") +
  facet_wrap(~ Random_ID, scales = "free") +
...
ggplot(df_fdbk_Ax_H, aes(x=Response, y=Response_H)) +
  geom_point(aes(color="Data Points", alpha = 0.5)) +
  geom_smooth(method = "lm", color="blue", aes(group=Random_ID)) +     
  labs(color="Legend") +
  facet_wrap(~ Random_ID, scales = "free") +
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
df_spin <- read.csv("/Users/marjan/Desktop/pilot_surprise_SeptOCT2023/Pilot_surprise/SUP_PRF_pilot_fdbk_bignrnd_typ_v3_mini_spin.csv")   # Prolific,non-random feedback, text only (no video/audio) with bigger PE, once per judge
...
df_spin <- df_spin[!is.na(df_spin$Random_ID), ]
merged_df_spinAX <- merge(df_fdbk_Ax_H, df_spin, by = "Random_ID")

# Calculate correlation by Random_ID
correlations_Ax_miniSPIN <- merged_df_spinAX %>%
  group_by(Random_ID) %>%
...
subset_df <- subset_df %>%
  group_by(Random_ID) %>%
  filter(!(Random_ID %in% c("SUPPRF56321", "SUPPRF93027", "SUPPRF10753") & Response_H < 20))
...
# Exclude rows with NA in Random_ID from both data frames
df_spin <- df_spin[!is.na(df_spin$Random_ID), ]
merged_df <- merge(correlations_Ax_excludedoutliers, df_spin, by = "Random_ID")

# looking at the correltion between the correlation of subjtPE and anxiety, with mini_SPIN score
group_correlation <- cor(merged_df$correlation, merged_df$mini_SPIN_total, use = "complete.obs")

# Print the correlation
print(group_correlation)

p <- ggplot(merged_df, aes(x=correlation, y=mini_SPIN_total)) +
...
df_long_mini_SPIN <- subset_df %>%
  left_join(df_spin, by = "Random_ID")

model <- lm(Response_Ax ~ Response_SubjPE * mini_SPIN_total, data = df_long_mini_SPIN)
summary(model)
```

```{r echo=FALSE, warning=FALSE, message=FALSE}

model <- lm(Response_H ~ Response_SubjPE * mini_SPIN_total, data = df_long_mini_SPIN)
summary(model)
```
...
subset_df <- subset(df_long_mini_SPIN, mini_SPIN_total >= 6)
ids_to_exclude <- c("SUPPRF67412")

# Subset the data
subset_df <- subset_df[!subset_df$Random_ID %in% ids_to_exclude, ]
...
```
<br> <br>
So after keeping people with high mini_SPIN scores, the correlation becomes -0.22 from -0.17. <br>


\newpage <font size="3"> We will now look at the same data in a pilot with bigger prediction errors. Per judge we now provide a rating between 90-100 to have the maximum positive PE possible using the following experiment:https://app.gorilla.sc/admin/experiment/149662/design and task: surprise_task_PE_feedback_biggestPE (https://app.gorilla.sc/admin/task/690668/editor?version=1)
...
<br> <br> 
```{r echo=FALSE, warning=FALSE, message=FALSE}
df <- read.csv("/Users/marjan/Desktop/pilot_surprise_SeptOCT2023/Pilot_surprise/SUP_PRF_pilot_fdbk_biggestnrnd_typ_v3_task_main.csv")
...
# Subset the data
df <- df[!df$Random_ID %in% ids_to_exclude, ]
...
df_raw <- read.csv("/Users/marjan/Desktop/pilot_surprise_SeptOCT2023/Pilot_surprise/SUP_PRF_pilot_fdbk_biggestnrnd_typ_v3_task_main.csv")   # Prolific,non-random feedback, text only (no video/audio) with biggest PE (1 feedback between 90-100), once per judge, 41 subjects
...
# Calculate correlation by Random_ID for anxiety
correlations_Ax_excludedoutliers <- final_df %>%
  group_by(Random_ID) %>%
  summarise(correlation = cor(Response_Ax, Response_SubjPE, use = "complete.obs")) %>%
  mutate(label = paste("R = ", round(correlation, digits = 2)))  # create a label for the plot
...
```{r echo=FALSE, , warning=FALSE, message=FALSE, out.extra='width=\\textwidth, height=\\textheight, keepaspectratio'}
ggplot(final_df, aes(x = Response_Ax)) +
  geom_histogram(binwidth = 1, fill = "blue", color = "black", alpha = 0.5) +
  geom_rug(sides = "b", color = "red", size = 0.5) +
  # facet_wrap(~ Screen) +
  labs(title = "Histogram of Responses", x = "Anxiety Ratings", y = "Frequency") +
...
```{r echo=FALSE, , warning=FALSE, message=FALSE, out.extra='width=\\textwidth, height=\\textheight, keepaspectratio'}
ggplot(final_df, aes(x = Response_H)) +
  geom_histogram(binwidth = 1, fill = "blue", color = "black", alpha = 0.5) +
  geom_rug(sides = "b", color = "red", size = 0.5) +
  # facet_wrap(~ Screen) +
  labs(title = "Histogram of Responses", x = "Mood/Happiness Ratings", y = "Frequency") +
...
\newpage we will now look at the ICC outcome for anxiety
```{r echo=FALSE, warning=FALSE, message=FALSE}
library(irr)
library(lme4)
library(psych)

model <- lmer(Response_Ax ~ 1 + (1 | Random_ID), data = final_df)
# Extract variance components
var_components <- VarCorr(model)
subject_variance <- as.numeric(var_components[1])
residual_variance <- as.numeric(attr(var_components, "sc")^2)
...
\newpage 
```{r echo=FALSE, warning=FALSE, message=FALSE}
model <- lmer(Response_H ~ 1 + (1 | Random_ID), data = final_df)
...
# cor.test(final_df[final_df$Trial.Number==1, ]$Response_H, final_df[final_df3$Trial.Number==48, ]$Response_H)

# 
# # Create a list of unique Random_IDs that occur in either Trial.Number == 1 or Trial.Number == 48
# unique_ids <- unique(c(final_df$Random_ID[final_df$Trial.Number == 1], final_df$Random_ID[final_df$Trial.Number == 48]))
# # 
# # # Initialize a data frame to store the count of occurrences for each Random_ID in both trials
# count_df <- data.frame(Random_ID = unique_ids, Count_Trial_1 = NA, Count_Trial_48 = NA)
# # 
# # # Loop through each Random_ID and count occurrences in each trial
# for (id in unique_ids) {
#   count_df$Count_Trial_1[count_df$Random_ID == id] <- sum(final_df$Random_ID == id & final_df$Trial.Number == 1)
#   count_df$Count_Trial_48[count_df$Random_ID == id] <- sum(final_df$Random_ID == id & final_df$Trial.Number == 48)
# }
# # 
# # # Identify Random_IDs where the count is different between the two trials
# different_counts <- count_df[count_df$Count_Trial_1 != count_df$Count_Trial_48, ]
...
cor.test(final_df[final_df$Trial.Number==1, ]$Response_Ax, final_df[final_df$Trial.Number==47, ]$Response_Ax)
```

```{r echo=FALSE, warning=FALSE, message=FALSE}
print("Correlation between first and 47th/penultimate mood rating")
cor.test(final_df[final_df$Trial.Number==1, ]$Response_H, final_df[final_df$Trial.Number==47, ]$Response_H)
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
# Calculate correlation by Random_ID for anxiety
correlations_H_excludedoutliers <- final_df %>%
  group_by(Random_ID) %>%
  summarise(correlation = cor(Response_H, Response_SubjPE, use = "complete.obs")) %>%
  mutate(label = paste("R = ", round(correlation, digits = 2)))  # create a label for the plot
...
# Determine the range for the data if we want all plots to have the same limits, to know what limits to choose
x_range <- range(final_df$Response_H, na.rm = TRUE)
y_range <- range(final_df$Response_SubjPE, na.rm = TRUE)
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
df_spin <- read.csv("/Users/marjan/Desktop/pilot_surprise_SeptOCT2023/Pilot_surprise/SUP_PRF_pilot_fdbk_biggestnrnd_typ_v3_mini_spin.csv")   # Prolific,non-random feedback, text only (no video/audio) with bigger PE, once per judge
...
\newpage Please ignore the below for now.
```{r echo=FALSE, warning=FALSE, message=FALSE}
library(lme4)
# Fit a mixed effects model
# Assuming the fixed effect is 'fixed_var', the random effect is 'random_var', and the grouping variable is 'group_var'
# Assuming df is your data frame
model1 <- lmer(Response_H ~ Response_SubjPE + (1 | Random_ID) + (1 | Trial.Number), data = df_long_mini_SPIN)
summary(model1)
predictions_H <- predict(model1)

model2 <- lmer(Response_Ax ~ Response_SubjPE + (1 | Random_ID) + (1 | Trial.Number), data = df_long_mini_SPIN)
summary(model2)
predictions_Ax <- predict(model1)
