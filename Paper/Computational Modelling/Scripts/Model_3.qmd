...
$$
A_{it} = \beta_{0i} + \beta_{1i} \cdot \sum_{j=1}^{t} \gamma_i^{t - j} \cdot Ee_j + \beta_{2i} \cdot O_t 
$$

$$
M_{it} = \beta_{0i} + \beta_{1i} \cdot \sum_{j=1}^{t} \gamma_i^{t - j} \cdot Ee_j + \beta_{2i} \cdot O_t  
$$
...
model_3_anx_fit <- stan(model_code = model_3_anx, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

options(max.print = 99999)
# Print results
print(model_3_anx_fit)

# Save the model output as an .rds file
saveRDS(model_3_anx_fit, file = "model_3_anx_fit.rds")
```
...
# Compile and Fit the Model
model_3_mood_fit <- stan(model_code = model_3_mood, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

options(max.print = 9999)  # Set a higher limit

# Print results
print(model_3_mood_fit)

# Save the model output as an .rds file
saveRDS(model_3_mood_fit, file = "model_3_mood_fit.rds")
```
