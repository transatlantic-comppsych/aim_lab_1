...
## Model 8 (SA interactions)

$$
A_{it} = \beta_{0i} +\beta_{1i}\cdot  \sum_{j=1}^{t} \gamma_i^{t - j} \cdot PE_{pos,j} + \beta_{2i}\cdot \sum_{j=1}^{t} \gamma_i^{t - j} \cdot PE_{neg,j}  + \beta_{3i}\cdot O_t\ + \beta_{4i}\cdot SA + \beta_{5i}\cdot \sum_{j=1}^{t} \gamma_i^{t - j} \cdot PE_{pos,j}\cdot SA + \beta_{6i}\cdot \sum_{j=1}^{t} \gamma_i^{t - j} \cdot PE_{neg,j}\cdot SA + \beta_{6i}\cdot O_t\cdot SA 
$$

$$
M_{it} = \beta_{0i} +\beta_{1i}\cdot  \sum_{j=1}^{t} \gamma_i^{t - j} \cdot PE_{pos,j} + \beta_{2i}\cdot \sum_{j=1}^{t} \gamma_i^{t - j} \cdot PE_{neg,j}  + \beta_{3i}\cdot O_t\ + \beta_{4i}\cdot SA + \beta_{5i}\cdot \sum_{j=1}^{t} \gamma_i^{t - j} \cdot PE_{pos,j}\cdot SA + \beta_{6i}\cdot \sum_{j=1}^{t} \gamma_i^{t - j} \cdot PE_{neg,j}\cdot SA + \beta_{6i}\cdot O_t\cdot SA 
$$
...
model_8_SA_anx_fit <- stan(model_code = model_8_SA_anx, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

options(max.print = 9999)  

# Print results
print(model_8_SA_anx_fit)


saveRDS(model_8_SA_anx_fit, "model_8_SA_anx_fit.rds")
```
...
# Compile and Fit the Model
model_8_SA_mood_fit <- stan(model_code = model_8_SA_mood, data = stan_data, iter = 4000, warmup = 2000, chains = 4, seed = 123, cores = 4, control = list(adapt_delta = 0.95,
  max_treedepth = 10))

options(max.print = 9999)  # Set a higher limit

# Print results
print(model_8_SA_mood_fit)

saveRDS(model_8_SA_mood_fit, "model_8_SA_mood_fit.rds")
```
