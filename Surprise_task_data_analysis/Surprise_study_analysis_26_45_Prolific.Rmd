...
dfpro_25_46 <- read.csv("/Users/elenabagdades/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Desktop/GitHub/aim_lab_1/De_identified_data/surprise_study_prolific_26-45/SUP_PRF_26-45_task_main.csv")


# read CESD, mini_SPIN for all the studies

dfpro_25_46_spin <- read.csv("/Users/elenabagdades/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Desktop/GitHub/aim_lab_1/De_identified_data/surprise_study_prolific_26-45/SUP_PRF_26-45_mini_spin.csv")

dfpro_26_45_cesd <- read.csv("/Users/elenabagdades/Library/CloudStorage/OneDrive-UniversityCollegeLondon/Desktop/GitHub/aim_lab_1/De_identified_data/surprise_study_prolific_26-45/SUP_PRF_26-45_ces_d.csv")

```

```{r echo=FALSE, warning=FALSE, message=FALSE}
# extract CES-D scores
CESD_data_clean <- dfpro_26_45_cesd[, c(31, 33, 35, 37, 39, 41, 43, 45, 47,49,51,
                                 53, 55, 57, 59, 61, 63, 65, 67, 69, 70)]
...
dfpro_25_46$Trial.Number[dfpro_25_46$Display == "Trials "] <- dfpro_25_46$Trial.Number[dfpro_25_46$Display == "Trials "] + 4

dfpro_25_46 <- dfpro_25_46 %>%
...
dfpro_25_46_spin <- dfpro_25_46_spin %>%
  rename(
...
dfpro_26_45_selected <- dfpro_25_46[, c("Trial.Number", "Display", "Screen", "Response.Type", "Response", 
...
# dfpro_26_45_selected_cert <- subset(dfpro_26_45_selected, Screen == "Certainty rating ")
# names(dfpro_26_45_selected_cert)[names(dfpro_26_45_selected_cert) == "Screen"] <- "Screen_certainty"
# names(dfpro_26_45_selected_cert)[names(dfpro_26_45_selected_cert) == "Response"] <- "Response_certainty"
...
final_df_26_45_Pro <- final_df_26_45_Pro %>%
  left_join(dfpro_25_46_spin %>% dplyr::select(Random_ID, mini_SPIN_total), by = "Random_ID")
...
write.csv(final_df_26_45_Pro, "final_df_26_45_Pro_with_stresslevels.csv", row.names = FALSE)
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
# Check the amount of missing data in each group

correlations <- final_df_26_45_Pro %>%
  group_by(Random_ID) %>%
...
ggplot(final_df_26_45_Pro, aes(x = Response_m_hist, y = Response_pred, color = Social_Anxiety, group= Social_Anxiety)) +
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
correlations_Ax <- final_df_26_45_Pro %>%
  group_by(Random_ID) %>%
...
ggplot(final_df_26_45_Pro, aes(x = Response_SubjPE, y = Response_Ax, color = Social_Anxiety, group= Social_Anxiety)) +
  geom_point(aes(color=Social_Anxiety, alpha = 0.3)) +
  geom_smooth(method = "lm", color="blue", aes(group=Random_ID)) + 
  # labs(color="Legend") +
  theme_minimal() +
...
correlations_M <- final_df_26_45_Pro %>%
  group_by(Random_ID) %>%
...
ggplot(final_df_26_45_Pro, aes(x = Response_SubjPE, y = Response_H, color = Social_Anxiety, group= Social_Anxiety)) +
  geom_point(aes(color=Social_Anxiety, alpha = 0.3)) +
  geom_smooth(method = "lm", color="blue", aes(group=Random_ID)) + 
  # labs(color="Legend") +
  theme_minimal() +
...
dfpro_25_46_with_BL <- dfpro_25_46[, c("Trial.Number", "Display", "Screen", "Response.Type", "Response", 
                     "Reaction.Time", "Spreadsheet..Histogram", "Spreadsheet..Feedback", "Random_ID")]

dfpro_25_46_with_BL <- dfpro_25_46_with_BL %>%
  mutate(Response = as.numeric(as.character(Response))) %>%
  dplyr::filter(!is.na(Response))

dfpro_25_46_with_BL <- dfpro_25_46_with_BL %>% dplyr::filter(Response.Type != "info")

dfpro_25_46_with_BL$Screen <- ifelse(dfpro_25_46_with_BL$Trial.Number == 1 & (dfpro_25_46_with_BL$Display == "Mood " | dfpro_25_46_with_BL$Display == "Anxiety "), "BL_1",
                      ifelse(dfpro_25_46_with_BL$Trial.Number == 2 & (dfpro_25_46_with_BL$Display == "Mood " | dfpro_25_46_with_BL$Display == "Anxiety "), "BL_2", 
                             dfpro_25_46_with_BL$Screen))

dfpro_25_46_with_BL$m_hist <- ifelse(dfpro_25_46_with_BL$Spreadsheet..Histogram == "h1.png", 21,
                      ifelse(dfpro_25_46_with_BL$Spreadsheet..Histogram == "h2.png", 29,
                             ifelse(dfpro_25_46_with_BL$Spreadsheet..Histogram == "h3.png", 37,
                                    ifelse(dfpro_25_46_with_BL$Spreadsheet..Histogram == "h4.png", 46,
                                           ifelse(dfpro_25_46_with_BL$Spreadsheet..Histogram == "h5.png", 54,
                                                  ifelse(dfpro_25_46_with_BL$Spreadsheet..Histogram == "h6.png", 63,
                                                         ifelse(dfpro_25_46_with_BL$Spreadsheet..Histogram == "h7.png", 71,
                                                                ifelse(dfpro_25_46_with_BL$Spreadsheet..Histogram == "h8.png", 80, NA))))))))  
...
# # setting Mood and Anxiety trial numbers to match the trials starting from 1 (since we removed the baseline ones above)
dfpro_25_46_with_BL$Trial.Number <- ifelse(dfpro_25_46_with_BL$Display == 'Mood ' | dfpro_25_46_with_BL$Display == 'Anxiety ', dfpro_25_46_with_BL$Trial.Number - 2, dfpro_25_46_with_BL$Trial.Number)
# dfpro_25_46_with_BL$Display <- ifelse(dfpro_25_46_with_BL$Display == 'Mood ' | dfpro_25_46_with_BL$Display == 'Anxiety ', "Trials", dfpro_25_46_with_BL$Display)

final_dfpro_25_46_with_BL <- dfpro_25_46_with_BL[c("Trial.Number", "Display", "Screen", "Response", "Random_ID")]

final_dfpro_25_46_with_BL <- final_dfpro_25_46_with_BL %>%
  left_join(dfpro_25_46_spin %>% dplyr::select(Random_ID, mini_SPIN_total), by = "Random_ID")

# final_dfpro_25_46_with_BL <- final_dfpro_25_46_with_BL %>%
#   left_join(CESD_data_clean %>% dplyr::select(Random_ID, CESD_score, Depression_Threshold, Depression_status), by = "Random_ID")

final_dfpro_25_46_with_BL$Social_Anxiety <- ifelse(final_dfpro_25_46_with_BL$mini_SPIN_total >= 6, "high", "low")
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
dfpro_26_45_selected_BL_ax <- subset(final_dfpro_25_46_with_BL, Display == "Anxiety ")
dfpro_26_45_selected_BL_H <- subset(final_dfpro_25_46_with_BL,Display == "Mood ")

ggplot(dfpro_26_45_selected_BL_H, aes(x = Trial.Number, y = Response, color = Display, group = Display)) +
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
ggplot(dfpro_26_45_selected_BL_ax, aes(x = Trial.Number, y = Response, color = Display, group = Display)) +
  geom_line(color = "blue") +
  geom_vline(xintercept = 1, linetype = "dashed", color = "red") + # Adding a vertical line at Trial.Number == 1
  theme_minimal() +
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
ggplot(final_df_26_45_Pro, aes(x = Trial.Number, y = Response_pred)) +
  geom_line(color = "blue") +
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
onetrial_final_df_26_45_Pro <- subset(final_df_26_45_Pro, final_df_26_45_Pro$Trial.Number == 1)


ggplot(onetrial_final_df_26_45_Pro, aes(x = Social_Anxiety, y = Response_stress_level , fill = Social_Anxiety)) +
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
model1 <- lme4::lmer(Response_H ~ Response_SubjPE +  (Response_SubjPE | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model2 <- lme4::lmer(Response_H ~ Response_SubjPE +  mini_SPIN_total + (1 | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model3 <- lme4::lmer(Response_H ~ Response_SubjPE + mini_SPIN_total + (Response_SubjPE | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model4 <- lme4::lmer(Response_H ~ Response_SubjPE + Social_Anxiety + (Response_SubjPE | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
...
Response_SubjPE_range <- seq(min(final_df_26_45_Pro$Response_SubjPE), max(final_df_26_45_Pro$Response_SubjPE), length.out = 100)
...
ggplot(final_df_26_45_Pro, aes(x=Response_SubjPE, y=Response_H)) +
  ...
  xlab("Social PE") + 
  ylab("Mood") +
  ggtitle("", subtitle=paste("Estimated slopes of the association in n = ", length(unique(final_df_26_45_Pro$Random_ID)))) +
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
model1 <- lme4::lmer(Response_Ax ~ Response_SubjPE +  (Response_SubjPE | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model2 <- lme4::lmer(Response_Ax ~ Response_SubjPE +  mini_SPIN_total + (1 | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model3 <- lme4::lmer(Response_Ax ~ Response_SubjPE + mini_SPIN_total + (Response_SubjPE | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model4 <- lme4::lmer(Response_Ax ~ Response_SubjPE + Social_Anxiety + (Response_SubjPE | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
...
ggplot(final_df_26_45_Pro, aes(x=Response_SubjPE, y=Response_Ax)) +
  # Adding points from your dataset
  # geom_point() +
  # Adding a regression line
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
model1 <- lme4::lmer(Response_H ~ Response_pred +  (Response_pred | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model2 <- lme4::lmer(Response_H ~ Response_pred +  mini_SPIN_total + (1 | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model3 <- lme4::lmer(Response_H ~ Response_pred + mini_SPIN_total + (Response_pred | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model4 <- lme4::lmer(Response_H ~ Response_pred + Social_Anxiety + (Response_pred | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
model1 <- lme4::lmer(Response_Ax ~ Response_pred +  (Response_pred | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model2 <- lme4::lmer(Response_Ax ~ Response_pred +  mini_SPIN_total + (1 | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model3 <- lme4::lmer(Response_Ax ~ Response_pred + mini_SPIN_total + (Response_pred | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model4 <- lme4::lmer(Response_Ax ~ Response_pred + Social_Anxiety + (Response_pred | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
...
```{r}
model1 <- lme4::lmer(Response_H ~ Response_fdbk +  (Response_fdbk | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model2 <- lme4::lmer(Response_H ~ Response_fdbk +  mini_SPIN_total + (1 | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model3 <- lme4::lmer(Response_H ~ Response_fdbk + mini_SPIN_total + (Response_fdbk | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model4 <- lme4::lmer(Response_H ~ Response_fdbk + Social_Anxiety + (Response_fdbk | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
...
```{r}
model1 <- lme4::lmer(Response_Ax ~ Response_fdbk +  (Response_fdbk | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model2 <- lme4::lmer(Response_Ax ~ Response_fdbk +  mini_SPIN_total + (1 | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model3 <- lme4::lmer(Response_Ax ~ Response_fdbk + mini_SPIN_total + (Response_fdbk | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
model4 <- lme4::lmer(Response_Ax ~ Response_fdbk + Social_Anxiety + (Response_fdbk | Random_ID), data = final_df_26_45_Pro, control=lmerControl(optimizer="bobyqa"))
